...
describe('parseCodexJsonl', () => {
  ...
  test('parses Codex tool variants and turn context metadata', () => {
    const jsonl = [
      ...
      JSON.stringify({ type: 'response_item', payload: { type: 'message', role: 'user', content: [{ type: 'input_text', text: 'Search and run' }] } }),
    ...
    expect(exchanges).toHaveLength(1);
    expect(exchanges[0]).toMatchObject({ cwd: '/repo', model: 'gpt-5.2', provider: 'openai' });
