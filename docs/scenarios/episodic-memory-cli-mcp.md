# episodic-memory-cli-mcp: sync, search, and MCP share an isolated memory index

**What this covers**: The merged sync/source-boundary refactor from PR #84 through the real `bin/episodic-memory` CLI and MCP stdio interface.
...
the local cache. Use the scenario's own tmux session named `episodic-memory-cli-mcp`.
...
1. Seed one Claude transcript in `CLAUDE_CONFIG_DIR/projects/demo/session.jsonl`, then run the user-facing sync command in tmux:
   ...
   tmux new-session -d -s episodic-memory-cli-mcp -x 200 -y 50 "sh -c './bin/episodic-memory sync 2>\"$SCENARIO_ROOT/sync.stderr\"; rc=\$?; echo EXIT:\$rc; sleep 2'"
   ```
...
2. Wait for the sync session to exit, inspect the captured pane and stderr, and confirm the archive exists:
   ...
   ```bash
   set -euo pipefail
   while ! tmux capture-pane -t episodic-memory-cli-mcp -p 2>/dev/null | grep -F "EXIT:" >/dev/null; do sleep 0.2; done
   tmux capture-pane -t episodic-memory-cli-mcp -p
   grep -F "EXIT:0" <(tmux capture-pane -t episodic-memory-cli-mcp -p)
...
3. Seed one memory row directly into the isolated database as test fixture setup, then search through the real CLI:
   ...
   tmux new-session -d -s episodic-memory-cli-mcp -x 200 -y 50 "sh -c './bin/episodic-memory search \"sync indexing policy\" --limit 5 2>\"$SCENARIO_ROOT/search.stderr\"; rc=\$?; echo EXIT:\$rc; sleep 2'"
   while ! tmux capture-pane -t episodic-memory-cli-mcp -p 2>/dev/null | grep -F "EXIT:" >/dev/null; do sleep 0.2; done
   tmux capture-pane -t episodic-memory-cli-mcp -p | tee "$SCENARIO_ROOT/search-pane.txt"
   ```
...
5. Confirm the intentionally unsupported date options fail explicitly:
   ...
   ```bash
   set -euo pipefail
   if ./bin/episodic-memory search "sync" --after 2026-01-01 >"$SCENARIO_ROOT/unsupported.txt" 2>&1; then exit 1; fi
   grep -F -- "--after/--before are not yet supported" "$SCENARIO_ROOT/unsupported.txt"
   ```

6. Drive the real `bin/episodic-memory mcp` stdio interface and call the exposed search tool:

   ```bash
   set -euo pipefail
   tmux new-session -d -s episodic-memory-cli-mcp -x 200 -y 50 "sh -c 'PLUGIN_ROOT=\"$PWD\" bun -e '\''import { Client } from "@modelcontextprotocol/sdk/client/index.js"; import { StdioClientTransport } from "@modelcontextprotocol/sdk/client/stdio.js"; const transport = new StdioClientTransport({ command: process.cwd() + "/bin/episodic-memory", args: ["mcp"], env: { ...process.env, PLUGIN_ROOT: process.cwd() } }); const client = new Client({ name: "episodic-memory-scenario", version: "1.0.0" }); await client.connect(transport); const listed = await client.listTools(); const result = await client.callTool({ name: "search", arguments: { query: "sync indexing policy", limit: 5 } }); console.log(JSON.stringify({ tools: listed.tools.map((tool) => tool.name), result }, null, 2)); await client.close();'\'' 2>\"$SCENARIO_ROOT/mcp.stderr\"; rc=\$?; echo EXIT:\$rc; sleep 2'"
   while ! tmux capture-pane -t episodic-memory-cli-mcp -p 2>/dev/null | grep -F "EXIT:" >/dev/null; do sleep 0.2; done
   tmux capture-pane -t episodic-memory-cli-mcp -p | tee "$SCENARIO_ROOT/mcp-pane.txt"
   ```
...
```bash
tmux kill-session -t episodic-memory-cli-mcp 2>/dev/null || true
rm -r "$SCENARIO_ROOT"
```
...
- Always rebuild before starting the tmux session; `bin/episodic-memory` executes tracked `dist/` artifacts.
