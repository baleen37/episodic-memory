# Dual-Runtime Plugin Compatibility 설계

날짜: 2026-06-21

## 문제

memmem은 Claude Code와 Codex 양쪽에서 같은 기능을 제공해야 한다. 하지만 두 런타임의 plugin packaging, marketplace, cache, update semantics가 다르다.
...
따라서 본 작업은 깨진 상태 복구가 아니라, dual-runtime 배포를 지속 가능하게 만드는 hardening이다.
...
런타임별 manifest는 분리 유지하고, 공통 metadata와 검증을 강화한다.
...
런타임별 package를 완전히 분리하지도 않는다. memmem의 핵심 기능은 같은 MCP server, 같은 CLI, 같은 skills/hooks를 공유하므로 package 분리는 복제와 release drift를 늘린다.
...
이 형태는 두 런타임 모두에서 plugin root 기준 실행으로 이해하기 쉽다. MCP server는 다음 원칙을 유지한다.
...
Claude와 Codex update semantics를 분리해서 문서화한다.
...
2. dual-runtime manifest drift check script를 추가한다.
